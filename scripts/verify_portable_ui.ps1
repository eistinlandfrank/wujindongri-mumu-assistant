param([string]$OutputRoot = 'release_v5_64', [string]$Device = '')
$ErrorActionPreference = 'Stop'
$repo = Split-Path $PSScriptRoot -Parent
$release = [IO.Path]::GetFullPath((Join-Path $repo $OutputRoot))
$exe = Join-Path $release 'WJDRMuMuAssistant_v5.64.0_Portable.exe'
$digest = (Get-Content -LiteralPath (Join-Path $release 'portable_payload/payload.sha256') -Raw).Trim()
$runtime = Join-Path $env:LOCALAPPDATA ("WJDRMuMuAssistant/portable/5.64.0-" + $digest.Substring(0,12))
$env:WJDR_QA_PAGE = '4'
$env:WJDR_QA_SCREENSHOT = Join-Path $repo 'evidence/milestone-181-ui-multidevice/portable-boot.png'
...
if ($Device) {
    $env:WJDR_QA_SCREENSHOT = Join-Path $repo 'evidence/milestone-181-ui-multidevice/portable-second.png'
...
while ([DateTime]::UtcNow -lt $deadline) {
    ...
    $windows = @(Get-Process -Name WJDRMuMuAssistant -ErrorAction SilentlyContinue | Where-Object {
        $_.Path -eq (Join-Path $runtime 'WJDRMuMuAssistant.exe') -and $_.MainWindowTitle -like '*5.64.0*127.0.0.1:*' -and
