...
## Current status
- Version5.67.0 / milestone184: fixes mixed first-pair same-target modal stopping before Cancel. Sliding fresh confirmation up to4 additional frames/4s scheduling budget; unchanged exact matching, stop checks, ledger and three-conflict cap. Live2号 with one real positive frame withheld proved07:53:46 Cancel→matching20 release→07:54:10 own→07:55:53 green march→blue remainder→07:56:57 next own. Portable boot11.4s and five-module/assets match. Launch-only QA helper uses2cycles; normal starts uncapped, no helper packaged.0号 worker preserved.
- Version5.66.0: milestone183 live2号 proved own rally→green march→complete blue-only remainder→next owned rally in normal uncapped mode. User revised next-cycle rule: do not wait for blue marching/returning to clear; require two known-world frames, complete blue circular icon coverage and free numeric slot. Missing/green/unknown icons still block. Do not claim all troops home. 0号 changed to equipment page and was not forcibly recovered. See current beast-rally.md.
- Version5.65.0 adds per-account one-shot schedules (milestone182). Real16448 timer→fresh identity→lease→read-only callback→finished passed, with cancellation and no duplicate trigger. No actual user plan or game input was created. Bound account window/PC must stay awake; busy skips, >30s late misses, F8 cancels unstarted plans. Portable source/assets match; one-click boot11.55s, second account7.61s. Scheduling does not replace Beast lifecycle safety. See references/ui-multi-device.md.
- Version5.64.0 UI/multi-device release: milestone181 verified eight pages, two real1440×2560 devices, isolated frames/logs and four-process account writes. Portable one-click boot passed10.47s, second window6.36s. Use references/ui-multi-device.md for focused UI gates. Do not infer multi-hour simultaneous game-task or other-machine acceptance. Milestone180 same-target Cancel/re-search lifecycle remains the Beast evidence; QA flags are launch-only. Never Confirm same-target warnings. Allied-overlap acceptance remains pending. Inspect fresh processes; historical PIDs are not authority.
