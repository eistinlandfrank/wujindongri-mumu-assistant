...
## Current status
- Source candidate5.63.0; milestone180 reproduced the exact same-target warning with a user-coordinated20s race, then live-proved Cancel→restored formation→matching unsent reservation release→new Search→owned rally→march→idle. QA caps and race pause are launch-only, never retained for normal starts. Never Confirm this warning. Three consecutive conflicts cap re-search; success resets it. Allied-overlap live acceptance and new portable boot remain pending. Inspect fresh process/log state; historical PIDs are not active-worker authority.
